...
angular.module('hoge').factory('polylines', ($interval, width, height, delay, count) => {
  const n = 5,
        polylines = [];
  for (let i = 0; i < n; ++i) {
    polylines.push({
...
angular.module('hoge').directive('main', (Path) => {
  return {
    restrict: 'E',
    ...
    controllerAs: 'main',
    controller: class {
      constructor(width, height, circles, ellipses, rects, lines, texts, paths, polygons, polylines) {
        this.width = width;
        this.height = height;
